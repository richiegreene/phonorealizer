...
def create_main_window():
    ...
    def handle_file_selection(sender, app_data):
        ...
        partials = analyze_audio(file_path)
        if partials:
            dpg.set_value("status_text", f"Analysis complete for: {file_path}")
            visualize_partials(file_path, partials)
            dpg.set_item_user_data("export_button", (partials, file_path))
            dpg.set_item_user_data("synth_button", (partials, file_path))
    ...
    def export_data(sender, app_data, user_data):
    ...
    def synthesize_to_wav(sender, app_data, user_data):
        partials, file_path = user_data
        if not file_path:
...
def create_main_window():
    ...
    def handle_file_selection(sender, app_data):
        ...
        if partials:
            ...
            dpg.set_item_user_data("export_full_button", (partials, file_path, sr, duration))
            dpg.set_item_user_data("export_partials_button", (partials, file_path, sr, duration))
    ...
    def visualize_partials(file_path, partials):
        ...
        with dpg.plot(label="Spectrogram", height=-1, width=-1, parent="main_window", tag="spectrogram_plot"):
            ...
            for i, harmonic in enumerate(partials):
                if harmonic:
                    times, freqs, _ = zip(*harmonic)
                    dpg.add_line_series(x=list(times), y=list(freqs), label=f"Harmonic {i+1}", parent=y_axis, tag=f"harmonic_line_{i+1}")
    ...
    def export_selected_partials(sender, app_data, user_data):
        ...
        exported_count = 0
        for i, harmonic in enumerate(partials):
            harmonic_tag = f"harmonic_line_{i+1}"
            if dpg.does_item_exist(harmonic_tag) and dpg.is_item_shown(harmonic_tag):
                output_path = os.path.join(output_dir, f"harmonic_{i+1}.wav")
                # synthesize_from_partials expects a list of partials, so wrap the single harmonic
